...
__global__ void histo_private_kernel(char *data, int length, int *histo) {
    ...
    // We promote the private copy  of block 0 into a public copy
    // that will hold the total results from all block
    if(blockIdx.x > 0) {
        __syncthreads();
...
int main() {
    ...
    histo_private_kernel<<<BLOCKS, THREADS>>>(d_data, data_size, d_histo);
